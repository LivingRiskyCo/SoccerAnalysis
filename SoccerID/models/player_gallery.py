...
try:
    from ...utils.json_utils import safe_json_load, safe_json_save, JSONCorruptionError
    JSON_UTILS_AVAILABLE = True
except ImportError:
    try:
        from json_utils import safe_json_load, safe_json_save, JSONCorruptionError  # Fallback
        JSON_UTILS_AVAILABLE = True
    except ImportError:
    ...
    def match_player(self, 
        ...
        for player_id, profile in self.players.items():
            try:
                    ...
                    if total_weight > 0:
                        ...
                        if log_matching_details and 'foot' in feature_contributions:
                            ...
                            if foot_weight > 0.15:  # Only log if foot features have significant weight
                                logger.info(f"  👟 Foot features contributed {foot_weight*100:.1f}% to match with {profile.name} (foot_sim={foot_contrib:.3f}, final_sim={weighted_avg:.3f})")
                        ...
                        similarity = 0.7 * similarity + 0.3 * max_similarity
                    else:
                ...
                # DISABLED BY DEFAULT: Only log if debug mode is enabled via environment variable or debug flag
                # Set SOCCER_ANALYSIS_DEBUG=1 to enable diagnostic logging
