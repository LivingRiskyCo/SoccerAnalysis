...
class PlaybackViewer:
    def __init__(self, root, video_path=None, csv_path=None, initial_frame=None, highlight_ids=None, 
        ...
        # Ball trajectory and analytics (initialize early to prevent AttributeError)
    ...
    def create_widgets(self):
        ...
        # Force immediate update to ensure content is visible
        self.root.after(50, lambda: update_scroll_viz_width() if callable(update_scroll_viz_width) else None)
        ...
        if self.comparison_mode:
            ...
            # Initially allow propagation so container sizes based on available space
            self.canvas_container.grid_propagate(True)
            # Force initial update to make canvas visible
                    ...
                    if display_width > 1 and display_height > 1:
                        ...
                        if max_width > 0 and max_height > 0:
                            ...
                            if abs(current_width - max_width) > 5 or abs(current_height - max_height) > 5:  # Only update if change is significant
                                # Disable propagation when we set explicit size
                                ...
                                # If size is already correct, ensure propagation is off
                                if self.canvas_container.grid_propagate():
                                    self.canvas_container.grid_propagate(False)
                except:
                    pass
            ...
            # Create canvas first to ensure it exists
            ...
            # Force immediate size update to ensure canvas is visible
            self.root.after(100, lambda: update_canvas_width(force=True))
            # Bind mouse events for zoom and pan
    ...
    def _load_csv_worker(self, filename):
        ...
        # Note: show_ball_trajectory and show_ball_speed_overlay are initialized in __init__ to prevent AttributeError
    ...
    def _render_analytics_from_csv(self, display_frame, frame_num):
        ...
        if analytics_pos == "with_player":
            ...
            if frame_num in self.player_data and frame_num in self.analytics_data:
    ...
    def _render_analytics_panel(self, display_frame, frame_num, position):
        ...
        # CRITICAL: Check if analytics are enabled
        if not hasattr(self, 'show_analytics') or not self.show_analytics.get():
            return display_frame
    ...
    def mark_event(self, event_type: str):
        """Mark an event at the current frame (keyboard shortcut handler)"""
        if not self.video_path or self.current_frame_num < 0:
            return
        
        # Use the same logic as mark_event_at_current_frame to ensure consistency
        try:
            ...
            event_type_map = {
                ...
                'shot': EventType.SHOT,
                'pass': EventType.PASS,
                'foul': EventType.CUSTOM,  # Foul not in enum, use custom
                'corner': EventType.CORNER,
                'free_kick': EventType.FREE_KICK,
                'save': EventType.SAVE
            }
            event_type_enum = event_type_map.get(event_type.lower(), EventType.CUSTOM)
        ...
        # Create marker and add to event marker system
        ...
        # Add to event marker system (this is what gets saved)
        self.event_marker_system.add_marker(marker)
        
        # Also add to event tracker if available (for timeline display)
        if self.event_tracker:
            try:
                self.event_tracker.add_event(
                    ...
                    player_name=player_name,
                    team=team,
                    x_position=position[0] if position else None,
                    y_position=position[1] if position else None
                )
            except:
                pass  # Event tracker might not support all event types
    ...
    def save_event_markers(self):
        ...
        if not self.event_marker_system or not self.event_marker_system.markers:
        ...
        try:
            if self.video_path:
                ...
                if default_path and os.path.exists(default_path):
                    messagebox.showinfo("Markers Saved", 
                                      f"Saved {len(self.event_marker_system.markers)} markers to:\n{default_path}")
                else:
                    messagebox.showerror("Save Failed", "Could not save event markers. Please check file permissions.")
            else:
                ...
                if filename:
                    saved_path = self.event_marker_system.save_to_file(filename)
            ...
            messagebox.showerror("Save Error", f"Error saving event markers: {e}")
            import traceback
            traceback.print_exc()
